...
(async function openFieldViewer(){
 ...
 try{
  ...
  let [html,patch]=await Promise.all([fetchText(source),fetchText(revision)]);clearTimeout(timer);
  if(!/<html[\s>]/i.test(html)||!/<script[\s>]/i.test(html)||!patch.includes('fieldRevision28'))throw new Error('배치도 형식 오류');
  ...
  html=html.replace(/<\/body>/i,'<script>'+patch.replace(/<\/script/gi,'<\\/script')+'</script></body>');
