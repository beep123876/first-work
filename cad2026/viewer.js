...
(async function openFieldViewer(){
 ...
 try{
  ...
  const countPatch=`(function(){
   if(!window.__hcfFoundationAssignments||typeof updateStaffCount!=='function')return;
   const previous=updateStaffCount;
   updateStaffCount=function(){previous();const active=scene.staff_phases[$('phase').value]||[];
    if(!active.some(p=>p.id==='F22'))return;
    if(scene.operation_roster.foundation.filter(foundationOnDuty).some(p=>p.person_id==='F22'))return;
