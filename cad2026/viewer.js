...
(async function openFieldViewer(){
 ...
 try{
  ...
  let [html,patch,sync]=await Promise.all([fetchText(source),fetchText(revision),fetchText(roster)]);clearTimeout(timer);
  if(!/<html[\s>]/i.test(html)||!/<script[\s>]/i.test(html)||!patch.includes('fieldRevision28')||!sync.includes('syncFieldRoster281'))throw new Error('배치도 형식 오류');
